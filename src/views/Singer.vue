<template>
  <div class="singer">
    <ScrollView ref="scrollView">
      <ul class="list-wrapper">
        <li class="list-group" v-for="(value,index) in list" :key="index" ref="group">
          ...
          <ul>
            <li class="group-item" v-for="obj in value" :key="obj.id" @click.stop="switchSinger(obj.id)">
    ...
    <ul class="list-keys">
      <!-- <li v-for="(key,index) in keys" :key="key" @click.stop="keyDowm(index)"
      :class="{'active' : currentIndex === index}">{{key}}</li> -->
       <li v-for="(key,index) in keys" :key="key" :data-index="index" @touchstart.stop.prevent="touchstart"  @touchmove.stop.prevent="touchmove"
      :class="{'active' : currentIndex === index}">{{key}}</li>
    </ul>
...
.singer{
  ...
  .list-keys{
    ...
    li{
      @include font_color();
      @include font_size($font_medium-s);
      padding: 3px 0px;
      &.active{
